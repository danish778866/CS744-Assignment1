...
object PageRankFinal {
  def main(args: Array[String]) {
    ...
    var ranks = rankTemp.distinct().partitionBy(new HashPartitioner(5))
    var links = filteredFile2.map(x => (x.split("\t")(0), x.split("\t")(1)))
    var linksCombine = links.groupByKey().partitionBy(new HashPartitioner(5))
    for(a <- 1 to 10) {
        val contribs = linksCombine.join(ranks).values.flatMap{
          case(urls, rank) => val size_t = urls.size
          urls.map(url => (url,rank/size_t))
        }.reduceByKey{case(x,y) => x+y}
        ranks = contribs.map{case(x,y) => (x,0.15 + 0.85 * y)}.partitionBy(new HashPartitioner(5))
    }
